...
def main():
	...
	seg_tree = SegmentTree(lambda x, y: x + y, 0, a)

	for _ in range(q):
		match [int(i) for i in input().split()]:
			case 1, x:
				left_value = seg_tree.query(x - 1, x)
				right_value = seg_tree.query(x, x + 1)
				seg_tree.update(x - 1, right_value)
				seg_tree.update(x, left_value)
			case 2, l, r:
